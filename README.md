...
Before creating a draft, **Plan with orchestrator** can persist a classic or managed planning conversation under `project-plans/assistance/` using `apb.plan-assistance.v1`. Messages may be sent to the inference provider configured for Hermes. Treat them as provider-visible and do not include secrets. Suggestions cannot save, approve, launch, mutate control state, or execute commands; only the operator's explicit **Create persisted draft from proposal** action invokes the existing plan-create transaction.
