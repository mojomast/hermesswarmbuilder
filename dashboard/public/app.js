...
function renderPlanning(){renderPlanList();renderPlanEditor();renderPlanReview();planStatus(planning.state);planError(planning.error);applyPlanningMobilePane()}
...
function openPlanner(){planning.open=true;$('planningWorkspace').hidden=false;document.body.classList.add('planner-open');refreshPlans(!planning.dirty).then(()=>{$('planningWorkspace')?.querySelector('button,input,textarea')?.focus()})}
...
document.addEventListener('input',e=>{if(!e.target.closest('#projectPlanForm'))return;planning.dirty=true;planning.fieldErrors[e.target.name]='';planStatus('dirty','unsaved');planError('')});
...
window.addEventListener('keydown',e=>{const typing=['INPUT','TEXTAREA','SELECT'].includes(document.activeElement?.tagName); if(e.key==='/'&&!planning.open&&document.activeElement!==$('globalFilter')){e.preventDefault();$('globalFilter')?.focus()} if(!typing&&!planning.open&&e.key.toLowerCase()==='d'){e.preventDefault();cycleDensity()} if(e.key==='Escape'){if(planning.open)closePlanner();else $('detailDrawer')&&($('detailDrawer').hidden=true)}});
