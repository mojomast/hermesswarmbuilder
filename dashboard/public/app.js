...
function renderIterationControls(d,acceptedCount){const attrs=iterationActionAttrs(d);return `<div class="iteration-controlbar"><button class="primary" data-continue-run="${esc(d.runId)}" ${attrs}>Resume from this iteration</button><button data-fork-run="${esc(d.runId)}" ${attrs}>Fork with change</button><button class="accepted" data-use-run-direction="${esc(d.runId)}" ${attrs} ${acceptedCount?'':'title="No accepted features recorded yet"'}>Use accepted direction</button></div>`}
...
document.addEventListener('change',e=>{const input=e.target.closest('[data-section-visible]');if(!input)return; if(input.checked)model.hiddenSections.delete(input.dataset.sectionVisible);else model.hiddenSections.add(input.dataset.sectionVisible);persistLayoutPrefs();applyDashboardLayoutPrefs();});
...
document.addEventListener('click',async e=>{
  ...
  const payloadFromIterationButton=(btn,kind)=>{const runId=btn.dataset.continueRun||btn.dataset.forkRun||btn.dataset.useRunDirection;const iter=model.iterationDetail?.runId===runId?model.iterationDetail:(model.iterations||[]).find(x=>x.runId===runId||x.id===btn.dataset.sourceIteration)||{};return {sourceRunId:runId,runId,sourceIterationId:btn.dataset.sourceIteration||iter.id||null,repoPath:btn.dataset.repoPath||iter.repoPath||null,objective:kind==='use'?`Use run ${runId} as the next baseline direction`:kind==='fork'?`Fork run ${runId} with operator change`:`Continue improving run ${runId}`,changeText:kind==='use'?'Use the accepted commit as the baseline for one bounded objective-linked generation.':'Continue with one bounded objective-linked improvement while preserving prior evidence and constraints.'};};
  if(contBtn){const cont=contBtn.dataset.continueRun;try{await postCommand('continue-from-iteration',payloadFromIterationButton(contBtn,'continue'))}catch(err){showSteeringError(err)} return;}
  const forkBtn=e.target.closest('[data-fork-run]');
  if(forkBtn){const fork=forkBtn.dataset.forkRun;const change=prompt('Change request for forked iteration?','Try a bounded alternate direction while preserving tests, evidence, accessibility, and repo continuity.'); if(change!==null)try{await postCommand('fork-from-iteration',{...payloadFromIterationButton(forkBtn,'fork'),change})}catch(err){showSteeringError(err)} return;}
  const useRunBtn=e.target.closest('[data-use-run-direction]');
  if(useRunBtn){try{await postCommand('use-as-next-direction',payloadFromIterationButton(useRunBtn,'use'))}catch(err){showSteeringError(err)} return;}
...
window.addEventListener('keydown',e=>{const typing=['INPUT','TEXTAREA','SELECT'].includes(document.activeElement?.tagName); if(e.key==='/'&&document.activeElement!==$('globalFilter')){e.preventDefault();$('globalFilter')?.focus()} if(!typing&&e.key.toLowerCase()==='d'){e.preventDefault();cycleDensity()} if(e.key==='Escape'){$('detailDrawer')&&($('detailDrawer').hidden=true)}});
