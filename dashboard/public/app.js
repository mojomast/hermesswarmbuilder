...
function ingestEvents(list){for(const raw of list||[]){const e=normalizeEvent(raw); if(model.seenEventIds.has(e.id))continue; model.seenEventIds.add(e.id); model.events.push(e); const t=extractTool(e); if(t)model.toolCalls.set(t.id,t)} model.events=model.events.slice(-1000); model.seenEventIds=new Set(model.events.map(e=>e.id)); if(model.toolCalls.size>500){const keep=[...model.toolCalls.entries()].slice(-500); model.toolCalls=new Map(keep)}}
...
function renderSteering(){
  ...
  const control=model.control||{}, queue=model.queue?.items||[], gates=model.gates?.gates||[], audit=model.audit||[];
  const pinned=queue.find(x=>x.id===control.pinnedQueueItemId)||queue.find(x=>x.status==='pinned');
  const steering=(control.activeSteering||[]).slice(0,3);
  c.innerHTML=`<div class="steering-head"><div><b>Steering Cockpit</b><span class="muted"> decide what gets built next, hold/resume the swarm, and preserve tournament ideas as queue items</span></div><div class="steering-actions"><button data-command="pause">Pause checkpoint</button><button data-command="resume">Resume</button><button data-command="hold">Hold new runs</button><button data-command="run-now">Run next tick</button></div></div><div class="steering-grid"><section class="steering-card"><h3>Next build</h3><p>${pinned?`Pinned: <b>${esc(pinned.title)}</b>`:'No pinned queue item. The runner will use active idea files or autonomous tournament fallback.'}</p><form id="queueForm" class="stack-form"><input name="title" placeholder="Project title / Hermes self-idea" required><textarea name="objective" placeholder="Objective: what should the swarm build or improve?" required></textarea><textarea name="constraints" placeholder="Constraints, one per line: local-only, tests, benchmark, UX gates…"></textarea><label><input type="checkbox" name="pin" checked> Pin as next build and export to idea.txt</label><button>Add to queue</button></form></section><section class="steering-card"><h3>Queue</h3><div class="queue-list">${queue.slice(0,8).map(item=>`<div class="queue-row ${item.id===control.pinnedQueueItemId?'active':''}"><div><b>${esc(item.title)}</b><div class="row-sub">${esc(item.status)} · priority ${esc(item.priority||0)} · ${esc(item.source||'user')}</div></div><div><button data-pin-queue="${esc(item.id)}">Pin</button><button data-archive-queue="${esc(item.id)}">Archive</button></div></div>`).join('')||'<div class="empty">No queue items yet.</div>'}</div></section><section class="steering-card"><h3>Active steering</h3><form id="steerForm" class="stack-form"><textarea name="text" placeholder="Steer current/next run: e.g. keep improving Prism Pachinko; prioritize browser smoke and gate evidence." required></textarea><select name="scope"><option value="next_run">next run</option><option value="current_run">current run</option><option value="global">global</option></select><button>Add directive</button></form><ul>${steering.map(s=>`<li><b>${esc(s.priority)}</b> ${esc(s.scope)} — ${esc(s.text)}</li>`).join('')||'<li class="muted">No active steering directives.</li>'}</ul></section><section class="steering-card"><h3>Gates & audit</h3><form id="gateForm" class="stack-form"><input name="description" placeholder="New acceptance gate"><input name="requiredEvidence" placeholder="Required evidence"><button>Add gate</button></form><div class="row-sub">${gates.length} gates · run admission <b>${esc(control.runAdmission||'enabled')}</b> · pause <b>${control.pause?.requested?'requested':'off'}</b></div><ol>${audit.slice(0,5).map(a=>`<li>${fmt(a.ts)} ${esc(a.action||a.type)} ${esc(a.result?.item?.title||a.result?.pinnedQueueItemId||'')}</li>`).join('')||'<li class="muted">No operator audit yet.</li>'}</ol></section></div>`;
}
...
async function loadRunResources(force=false){if(!model.selectedRunId){model.artifacts=[];model.logs=[];return} const now=Date.now(); if(!force&&now-model.lastResourceLoad<15000)return; model.lastResourceLoad=now; try{model.artifacts=await getJson(`/api/runs/${encodeURIComponent(model.selectedRunId)}/artifacts`)}catch{model.artifacts=[]} try{model.logs=await getJson(`/api/runs/${encodeURIComponent(model.selectedRunId)}/logs`)}catch{model.logs=[]}}
async function refresh(){try{const [state,runs,events,control,queue,gates,audit]=await Promise.all([getJson('/api/state'),getJson('/api/runs'),getJson('/api/events?limit=300'),getJson('/api/control'),getJson('/api/queue'),getJson('/api/gates'),getJson('/api/audit?limit=50')]); model.state=state; model.runs=runs; model.control=control; model.queue=queue; model.gates=gates; model.audit=audit; if(!model.selectedRunId)model.selectedRunId=model.state.currentRunId||(model.runs[0]?.id??null); ingestEvents(events); await loadRunResources(true); scheduleRender()}catch(e){if($('streamState')) $('streamState').textContent='API error'}}
let eventSource=null,pollTimer=null; function pushRaw(x){model.raw.push(x); model.raw=model.raw.slice(-80)} function startPolling(){if(pollTimer)return; pollTimer=setInterval(refresh,4000)} function connect(){try{if(eventSource)eventSource.close(); const es=new EventSource('/api/stream'); eventSource=es; es.addEventListener('open',()=>{$('streamState')&&($('streamState').textContent='SSE live'); if(pollTimer){clearInterval(pollTimer); pollTimer=null}}); es.addEventListener('state',e=>{const p=JSON.parse(e.data); pushRaw({type:'state',ts:new Date().toISOString(),payload:p}); if(!model.paused){model.state=p; if(!model.selectedRunId)model.selectedRunId=p.currentRunId; scheduleRender()}}); es.addEventListener('events',e=>{const p=JSON.parse(e.data); pushRaw({type:'events',ts:new Date().toISOString(),payload:p}); if(!model.paused){ingestEvents(p); scheduleRender()}}); es.addEventListener('heartbeat',e=>{pushRaw({type:'heartbeat',ts:new Date().toISOString(),payload:JSON.parse(e.data)}); if(!model.paused&&model.selectedRunId)loadRunResources(false).then(scheduleRender)}); es.onerror=()=>{$('streamState')&&($('streamState').textContent='SSE disconnected; polling'); es.close(); eventSource=null; startPolling()}}catch{startPolling()}}
...
document.addEventListener('submit',async e=>{const f=e.target; if(f?.id==='queueForm'){e.preventDefault(); const d=Object.fromEntries(new FormData(f).entries()); await postCommand('add-queue-item',{...d,pin:!!f.querySelector('[name="pin"]')?.checked,source:'dashboard'}); f.reset()} if(f?.id==='steerForm'){e.preventDefault(); const d=Object.fromEntries(new FormData(f).entries()); await postCommand('steer',d); f.reset()} if(f?.id==='gateForm'){e.preventDefault(); const d=Object.fromEntries(new FormData(f).entries()); await postCommand('add-gate',d); f.reset()}});
document.addEventListener('click',async e=>{const cmd=e.target.closest('[data-command]')?.dataset.command; if(cmd){await postCommand(cmd,{reason:'Dashboard operator command'}); return} const pin=e.target.closest('[data-pin-queue]')?.dataset.pinQueue; if(pin){await postCommand('pin-queue-item',{id:pin}); return} const arc=e.target.closest('[data-archive-queue]')?.dataset.archiveQueue; if(arc){await postCommand('archive-queue-item',{id:arc}); return}});
