...
async function renderRunJson(c){const iterId=model.selectedIterationId; if(iterId){try{const d=model.iterationDetail||await getJson(`/api/iterations/${encodeURIComponent(iterId)}`); model.iterationDetail=d; c.innerHTML=`<div class="inspector-section"><div class="mini-toolbar"><b>Iteration ${esc(d.id)}</b><button data-continue-run="${esc(d.runId)}">Continue</button><button data-fork-run="${esc(d.runId)}">Fork</button></div><div class="deck-grid"><div class="kv"><span>run</span><strong>${esc(d.runId)}</strong></div><div class="kv"><span>status</span><strong>${esc(d.status)}</strong></div><div class="kv"><span>gate</span><strong>${esc(d.gateStatus||'unknown')}</strong></div><div class="kv"><span>artifacts</span><strong>${esc((d.artifacts||[]).length)}</strong></div></div><h4>Variants</h4><pre class="raw-box">${esc(clipText(d.variants||[],12000))}</pre><h4>Evaluations / synthesis</h4><pre class="raw-box">${esc(clipText({evaluations:d.evaluations||[],synthesis:d.synthesis||null,gateDecisions:d.gateDecisions||[]},12000))}</pre><h4>Raw iteration detail</h4><pre class="raw-box">${esc(clipText(d,16000))}</pre></div>`; return}catch{}} if(!model.selectedRunId){c.innerHTML='<div class="empty">No run selected.</div>';return} try{c.innerHTML=`<pre class="raw-box">${esc(clipText(await getJson(`/api/runs/${encodeURIComponent(model.selectedRunId)}`),16000))}</pre>`}catch{c.innerHTML='<div class="empty">Run JSON unavailable.</div>'}}
