...
document.addEventListener('submit',async e=>{
  ...
  if(f?.id==='deblockForm'){
    e.preventDefault();
    const d=Object.fromEntries(new FormData(f).entries());
    try{await postCommand('deblock',d); f.reset()}
    catch(err){showSteeringError(err)}
    return;
  }
  if(f?.id==='steerForm'){
...
document.addEventListener('click',async e=>{
  ...
  if(operationsAction){
    ...
    if(operationsAction==='deblock'){
      model.hiddenSections.delete('steering');model.collapsedSections.delete('steering');persistLayoutPrefs();applyDashboardLayoutPrefs();skipNextFocusRestore=true;renderAll();focusOperationsDestination(document.querySelector('#deblockForm textarea')||$('steeringCockpit'),{block:'start'});return;
    }
  ...
  const askAdvice=e.target.closest('[data-ask-deblock-advice]');
  if(askAdvice){const form=askAdvice.closest('#deblockForm');const prompt=form?.querySelector('[name="prompt"]')?.value||'';try{await postCommand('deblock-advice',{prompt,runId:form?.querySelector('[name="runId"]')?.value||model.state?.currentRunId||null})}catch(err){showSteeringError(err)} return;}
  const approveAdvice=e.target.closest('[data-approve-deblock-advice]')?.dataset.approveDeblockAdvice;
  if(approveAdvice){try{await postCommand('approve-deblock-advice',{adviceId:approveAdvice})}catch(err){showSteeringError(err)} return;}
  const denyAdvice=e.target.closest('[data-deny-deblock-advice]')?.dataset.denyDeblockAdvice;
  if(denyAdvice){try{await postCommand('deny-deblock-advice',{adviceId:denyAdvice})}catch(err){showSteeringError(err)} return;}
