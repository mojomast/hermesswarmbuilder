...
export function renderTemporalMissionLiveMarkup(live, chamber) {
  ...
  const stageFocus = ({
    history: `<strong>Retained run history</strong><ul>${list(live.historicalRuns, (run) => `<li><code>${value(run.id)}</code> — ${value(run.status)} · ${value(run.date)}</li>`, "No retained historical runs.")}</ul>`,
    spec: `<strong>Approved plan binding</strong><div>Plan / revision: <code>${value(identity.planId)}</code> / <code>${value(identity.revision)}</code></div><div>Approval: <code>${value(identity.approvalId)}</code></div>`,
    draft: `<strong>Orchestrator activity</strong><div>Current agent / task: ${value(progress.currentAgent)} / ${value(progress.currentTask)}</div><ul>${checkpoints}</ul>`,
    arena: `<strong>Variant activity</strong><ul>${variants}</ul>`,
    eval: `<strong>Evaluation and validation activity</strong><div>Status / started / completed: ${value(progress.validation.status)} / ${value(progress.validation.startedAt)} / ${value(progress.validation.completedAt)}</div><ul>${validationResults}</ul>`,
    synth: `<strong>Synthesis / mashup activity</strong><div>Mashup / synthesis: ${value(progress.mashup?.status)}</div><ul>${artifacts}</ul>`,
    gate: `<strong>Gate and handoff evidence</strong><div>Handoff status: ${value(live.handoff?.state)}</div><ul>${gates}</ul>`
  })[chamber.id] || `<strong>Stage-specific observed activity</strong><div>Unavailable for this stage.</div>`;
  return `<div class="inspector-section"><div class="section-title"><span>Stage-specific observed activity</span><span class="status-badge status-${chamber.status}">${value(chamber.name)}</span></div><div class="section-card">${stageFocus}</div></div><div class="inspector-section"><div class="section-title"><span>Observed run progress</span><span class="status-badge status-${chamber.status}">${value(progress.state)}</span></div><div class="section-card"><div><strong>Stage:</strong> ${value(chamber.name)}</div><div><strong>Run status / phase:</strong> ${value(live.runStatus)} / ${value(live.phase)}</div><div><strong>Current agent / task:</strong> ${value(progress.currentAgent)} / ${value(progress.currentTask)}</div><div><strong>Active / terminal:</strong> ${value(progress.active)} / ${value(progress.terminal)}</div><div><strong>Started / updated / completed:</strong> ${value(progress.startedAt)} / ${value(progress.updatedAt)} / ${value(progress.completedAt)}</div><div><strong>Observed duration:</strong> ${value(progress.duration)}</div><div><strong>Plan / approval / launch:</strong> <code>${value(identity.planId)}</code> rev <code>${value(identity.revision)}</code> / <code>${value(identity.approvalId)}</code> / <code>${value(identity.launchId)}</code></div><div><strong>Request / run / iteration:</strong> <code>${value(identity.requestId)}</code> / <code>${value(identity.runId)}</code> / <code>${value(identity.iterationId)}</code></div></div></div><div class="inspector-section"><div class="section-title">Checkpoints, variants & mashup</div><div class="section-card"><div><strong>Mashup / synthesis:</strong> ${value(progress.mashup?.status)}</div><ul>${checkpoints}</ul><ul>${variants}</ul></div></div><div class="inspector-section"><div class="section-title">Validation</div><div class="section-card"><div><strong>Status / started / completed:</strong> ${value(progress.validation.status)} / ${value(progress.validation.startedAt)} / ${value(progress.validation.completedAt)}</div><ul>${validationResults}</ul></div></div><div class="inspector-section"><div class="section-title">Gate decisions</div><div class="section-card"><ul>${gates}</ul></div></div><div class="inspector-section"><div class="section-title">Blockers & suggested action</div><div class="section-card"><ul>${blockers}</ul></div></div><div class="inspector-section"><div class="section-title">Recent events & tool activity</div><div class="section-card"><ul>${activity}</ul></div></div><div class="inspector-section"><div class="section-title">Artifacts, handoff & evidence</div><div class="section-card"><div><strong>Handoff status:</strong> ${value(live.handoff?.state)}</div><ul>${artifacts}</ul></div></div>`;
}
