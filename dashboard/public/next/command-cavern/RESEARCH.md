...
2. **MDN, WebGL Best Practices**
   ...
   Applied: one batched draw for the cavern, tablet, and bounded landmark labels; no per-frame readback; a 500,000-pixel backbuffer budget; 18-22 ray steps; in-place texture updates; snapshot-diff invalidation; visibility sleep; and deletion of replaced resources.
...
4. **WebGL2 Fundamentals, Picking**
   ...
   Applied: pointer coordinates construct the same camera ray used by the fragment shader. The ray may intersect one of eight fixed exposed-cavern landmark slabs or the physical tablet front plane. Hits use the same world centers and bounds as the shader; tablet hits then transform through tablet-local coordinates into texture UV. No fullscreen or direct screen-region shortcut is used.
...
- The tectonic control tablet can collapse to a much smaller physical SDF plaque, exposing the cavern geometry behind it while retaining scene-native restore/open controls and the `T` keyboard toggle.
- UI glyphs are generated locally into a bounded 1600 x 900 inscription, then allocated within the queried `MAX_TEXTURE_SIZE`. Landscape uploads at 1600 x 900. Portrait now stacks the two 800 x 900 logical halves without anisotropic resampling into an 800 x 1800 texture; both upload dimensions are proportionally reduced only when the queried limit requires it. The portrait tablet's 2.25 physical aspect is fitted against both viewport axes, rather than clipping an oversized projection. Picking uses the same inverse packing and physical aspect. Subsequent updates use `texSubImage2D` rather than reallocating the texture. Linear sampling remains deliberate for fractional projection and bounded downscaling; nearest filtering would trade blur for unstable jagged glyph edges.
...
- When exposed, eight deterministic SDF landmark slabs pair those literal domains with their metaphors. Each label reports the truthful shader representation cap (`rendered/total`), an important actual record ID, and status. Events and tools share one inclusion landmark but preserve the selected record's literal kind. Label glyphs reuse the bounded tablet atlas, including the portrait packing transform; no DOM overlay, extra draw call, or per-entity shader array is introduced.
- Exposed-landmark picking analytically intersects the exact slab front plane and bounds. Pointer selection and arrow/Enter keyboard selection monitor an exact record identity across snapshot replacement. `O` or the engraved open control only restores the corresponding existing evidence, plan, or operation workspace; selection cannot dispatch an operation. Tablet picking retains its exact plane/UV inverse transform.
- Quality uses a fixed conservative 18-22 ray steps and a marked bounded density policy. A static full tablet starts at 100% CSS scale and may use at most 1,440,000 backing pixels; static minimized world inspection starts at 70% and 900,000 pixels; optional motion retains 45% and 500,000 pixels at no more than ten frames per second. Device pixel ratio remains capped at 1.25, backing dimensions remain capped at 1600 x 1200, and retry mode still caps scale at 38%. Unchanged heartbeat publications perform no rendering or texture upload, so the sharper static tier does not create a recurring frame loop or texture-upload stall. One uniform scale preserves exact CSS/backing aspect and therefore ray/pick agreement.
...
- Generic two-dimensional slabs use borders only; the former corner-to-corner decorative stroke was removed because it read as a geometry artifact.
- The local `update-gate` seed and allowed-key schema omit unsupported `status`. Gate outcomes remain exclusively in the separately validated `gate-decision` operation.
