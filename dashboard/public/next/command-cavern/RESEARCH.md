...
2. **MDN, WebGL Best Practices**
   ...
   Applied: one batched draw, no per-frame readback, a 500,000-pixel backbuffer budget, 18-22 ray steps, in-place texture updates, snapshot-diff invalidation, visibility sleep, and deletion of replaced resources.
...
- The tectonic control tablet can collapse to a much smaller physical SDF plaque, exposing the cavern geometry behind it while retaining a scene-native pickable restore control and `T` keyboard toggle.
...
- Quality uses a fixed conservative 18-22 ray steps with a 500,000-pixel backbuffer budget. Unchanged heartbeat publications perform no rendering or texture upload. The backing-store uses one uniform scale so its aspect ratio remains identical to the CSS viewport. Optional motion is capped at ten frames per second.
