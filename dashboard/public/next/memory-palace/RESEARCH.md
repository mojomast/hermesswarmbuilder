...
1. **Khronos Group, WebGL 2.0 Specification**  
   ...
   Applied: one indexed cuboid mesh is expanded with `drawElementsInstanced` into the cutaway archive, bays, desks, and rails. Workstation labels and controls are rasterized locally and composited as a WebGL texture, so normal mode remains one visible canvas.
...
3. **WebGL2 Fundamentals, 3D camera and perspective**  
   ...
   Applied: the archive uses a perspective projection and look-at orbit camera. Workstation controls use deterministic canvas-space hit regions in the WebGL-composited control plane rather than pretending their coordinates are operational measurements.
...
- A local 2D canvas creates label pixels only as a GPU texture source; it is never attached to the DOM. The visible presentation is WebGL2.
- Scene workstations use bounded hit regions generated with the same canvas pass that draws them. Pointer activation and arrow/Enter navigation use the identical control list; Tab exits the canvas.
...
- Command records use `pending`, `accepted intent`, `rejected`, and `outcome unknown`. No receipt is called an observed outcome.
