...
// WORLD_SPACE_CONTROL_FOLIO, RAY_PLANE_PICKING, FROZEN_CONFIRMATION_ENVELOPE.
...
const scene = {
  ...
  confirm: null, bridge: null, reducedMotion: matchMedia("(prefers-reduced-motion: reduce)").matches
};
...
function leaveSemantic() { if (!renderer?.available) return announce("WebGL2 is unavailable; semantic fallback remains active."); setSemanticEnabled(false); $("palace").focus(); announce("Scene-native control plane enabled."); }
...
async function handleSceneAction(action) {
  if (!action) return;
  if (action.startsWith("workspace:")) { scene.workspace = action.slice(10); scene.page = 0; scene.cursor = 0; scene.search = ""; renderer.invalidate(); return; }
  ...
  if (action.startsWith("command:")) { scene.command = action.slice(8); scene.operationDraft = json(defaultOperationPayload(scene.command)); scene.workspace = "operation-detail"; scene.cursor = 0; renderer.invalidate(); return; }
  ...
  if (action.startsWith("plan:")) return safeScene(async () => { selectedPlanId = action.slice(5); await client.getProjectPlan(selectedPlanId); scene.workspace = "plan-detail"; renderer.invalidate(); });
...
class PalaceRenderer {
  constructor(canvas){this.canvas=canvas;this.gl=null;this.available=false;this.lost=false;this.yaw=-.55;this.pitch=.34;this.distance=31;this.drag=null;this.pinch=null;this.instances=[];this.hotspots=[];this.controls=[];this.focus=0;this.snapshot=snapshot;this.overlay=document.createElement("canvas");this.ctx=this.overlay.getContext("2d");this.raf=0;this.dirty=true;this.overlayDirty=true;this.textureAllocated=false;this.bind();}
  ...
  bind(){this.canvas.addEventListener("webglcontextlost",(event)=>{event.preventDefault();this.lost=true;this.available=false;cancelAnimationFrame(this.raf);this.raf=0;activateFallback("WebGL context lost. Semantic fallback is active.");});this.canvas.addEventListener("webglcontextrestored",()=>{this.lost=false;this.start();announce("WebGL context restored. Return to 3D when ready.");});this.canvas.addEventListener("pointerdown",(event)=>{this.canvas.setPointerCapture(event.pointerId);this.drag={x:event.clientX,y:event.clientY,moved:false,id:event.pointerId};});this.canvas.addEventListener("pointermove",(event)=>{if(!this.drag||this.drag.id!==event.pointerId)return;const dx=event.clientX-this.drag.x,dy=event.clientY-this.drag.y;if(Math.abs(dx)+Math.abs(dy)>3)this.drag.moved=true;this.yaw+=dx*.006;this.pitch=Math.max(-.12,Math.min(1.05,this.pitch+dy*.005));this.drag.x=event.clientX;this.drag.y=event.clientY;this.invalidate(false);});this.canvas.addEventListener("pointerup",(event)=>{if(this.drag&&!this.drag.moved)this.pick(event.clientX,event.clientY);this.drag=null;});this.canvas.addEventListener("wheel",(event)=>{event.preventDefault();this.distance=Math.max(17,Math.min(52,this.distance+event.deltaY*.025));this.invalidate(false);},{passive:false});this.canvas.addEventListener("touchstart",(event)=>{if(event.touches.length===2)this.pinch=Math.hypot(event.touches[0].clientX-event.touches[1].clientX,event.touches[0].clientY-event.touches[1].clientY);},{passive:true});this.canvas.addEventListener("touchmove",(event)=>{if(event.touches.length!==2||!this.pinch)return;const distance=Math.hypot(event.touches[0].clientX-event.touches[1].clientX,event.touches[0].clientY-event.touches[1].clientY);this.distance=Math.max(17,Math.min(52,this.distance+(this.pinch-distance)*.04));this.pinch=distance;this.invalidate(false);},{passive:true});this.canvas.addEventListener("touchend",()=>{this.pinch=null;},{passive:true});document.addEventListener("visibilitychange",()=>{if(!document.hidden)this.invalidate();});window.addEventListener("resize",()=>this.invalidate());}
  ...
  initializeWorldFolio(){const gl=this.gl,vs=`#version 300 es\nconst vec3 P[6]=vec3[6](vec3(-8.,-1.,4.),vec3(8.,-1.,4.),vec3(-8.,8.,4.),vec3(-8.,8.,4.),vec3(8.,-1.,4.),vec3(8.,8.,4.));const vec2 U[6]=vec2[6](vec2(0.,1.),vec2(1.,1.),vec2(0.,0.),vec2(0.,0.),vec2(1.,1.),vec2(1.,0.));uniform mat4 vp;out vec2 uv;void main(){uv=U[gl_VertexID];gl_Position=vp*vec4(P[gl_VertexID],1.);}`,fs=`#version 300 es\nprecision mediump float;uniform sampler2D tex;in vec2 uv;out vec4 o;void main(){o=texture(tex,uv);}`;gl.deleteProgram(this.overlayProgram);this.overlayProgram=makeProgram(gl,vs,fs);this.folioVpLocation=gl.getUniformLocation(this.overlayProgram,"vp");this.textureAllocated=false;this.invalidate();}
  box(x,y,z,sx,sy,sz,color){this.instances.push(x,y,z,sx,sy,sz,...color);}
  buildScene(){this.instances=[];const stone=[.24,.23,.18],edge=[.48,.39,.22],floor=[.12,.15,.12];this.box(0,-.7,0,22,.55,22,floor);this.box(0,7.7,0,5,.4,5,edge);this.box(0,3.5,3.82,16.7,9.5,.22,[.14,.12,.08]);for(let level=0;level<4;level++)for(let i=0;i<42;i++){const a=-2.72+i*(5.44/41),r=9.2,y=.45+level*2.05,x=Math.sin(a)*r,z=Math.cos(a)*r;this.box(x,y,z,.42,1.55,.42,stone);if(i%3===0)this.box(x,y+.86,z,1.25,.18,.72,edge);}for(let i=0;i<28;i++){const a=-2.72+i*(5.44/27),r=6.3;this.box(Math.sin(a)*r,-.1,Math.cos(a)*r,.22,.45,5.6,stone);}for(let i=0;i<12;i++){const a=-2.5+i*.45;this.box(Math.sin(a)*4.2,.1,Math.cos(a)*4.2,1.25,.5,1.1,[.28,.24,.14]);}}
  setSnapshot(value){this.snapshot=value;this.invalidate();}
  ...
  cameraState(){const cp=Math.cos(this.pitch),eye=[Math.sin(this.yaw)*cp*this.distance,5+Math.sin(this.pitch)*this.distance,Math.cos(this.yaw)*cp*this.distance],target=[0,3,0];return {eye,target,vp:multiply(perspective(Math.PI/3,this.canvas.width/this.canvas.height,.1,100),lookAt(eye,target,[0,1,0]))};}
  camera(){return this.cameraState().vp;}
  button(x,y,w,h,label,action,style="normal"){const ctx=this.ctx,index=this.controls.length,focused=index===this.focus;ctx.fillStyle=focused?"#d9af59":style==="danger"?"#4b211c":"#202219";ctx.fillRect(x,y,w,h);ctx.strokeStyle=focused?"#fff1b7":style==="danger"?"#e46d5d":"#706546";ctx.strokeRect(x,y,w,h);ctx.fillStyle=focused?"#11130f":"#e9e0c7";ctx.font=`${Math.max(11,Math.floor(h*.35))}px monospace`;ctx.textBaseline="middle";ctx.fillText(String(label).slice(0,Math.max(8,Math.floor(w/9))),x+10,y+h/2);this.hotspots.push({x,y,w,h,action,index});this.controls.push({label,action});}
  header(title,subtitle){const c=this.ctx,w=this.overlay.width,scale=Math.max(.7,Math.min(1.35,w/1500));c.fillStyle="#090b08ee";c.fillRect(18*scale,18*scale,w-36*scale,88*scale);c.strokeStyle="#d9af59";c.strokeRect(18*scale,18*scale,w-36*scale,88*scale);c.fillStyle="#eadfbe";c.font=`700 ${23*scale}px monospace`;c.fillText(title,38*scale,52*scale);c.fillStyle="#afa58e";c.font=`${13*scale}px monospace`;c.fillText(subtitle,38*scale,80*scale);c.textAlign="right";c.fillStyle="#69c7bd";c.fillText(`${String(this.snapshot.connection?.status||"disconnected").toUpperCase()} / OBSERVED ${String(this.snapshot.state?.phase||"idle").toUpperCase()} / RUN ${currentRunId()||"none"}`,w-38*scale,80*scale);c.textAlign="left";return scale;}
  textBlock(value,x,y,lineCount=24,width=110){const c=this.ctx,s=Math.max(.7,Math.min(1.35,this.overlay.width/1500)),all=wrappedLines(value,width),pages=Math.max(1,Math.ceil(all.length/lineCount));scene.textPage=Math.min(scene.textPage||0,pages-1);c.fillStyle="#e9e0c7";c.font=`${12*s}px monospace`;all.slice(scene.textPage*lineCount,scene.textPage*lineCount+lineCount).forEach((line,index)=>c.fillText(line,x,(y+index*19*s)));c.fillStyle="#afa58e";c.fillText(`TEXT PAGE ${scene.textPage+1}/${pages} / ${all.length} LINES`,x,y+lineCount*19*s);this.button(this.overlay.width-350*s,this.overlay.height-82*s,145*s,40*s,"TEXT <","text-prev");this.button(this.overlay.width-190*s,this.overlay.height-82*s,145*s,40*s,"TEXT >","text-next");}
  drawRows(title,items,actions={}){const editing=scene.bridge?.kind==="search",c=this.ctx,w=this.overlay.width,h=this.overlay.height,s=this.header(`MEMORY PALACE / ${title}`,editing?`EDITING SEARCH${scene.bridge.focusLost?" / FOCUS LOST - DRAFT RETAINED":""}`:scene.notice),left=50*s,top=135*s,rowH=48*s,panelW=Math.min(w-100*s,1050*s);this.controls=[];this.hotspots=[];items.forEach((item,index)=>this.button(left,top+index*(rowH+7*s),panelW,rowH,item.label||item,item.action||actions[index]));this.button(left,h-78*s,150*s,42*s,"BACK","back");if(editing){this.button(left+165*s,h-78*s,150*s,42*s,"SAVE SEARCH","bridge-save");this.button(left+330*s,h-78*s,150*s,42*s,"CANCEL","bridge-cancel");}else{this.button(left+165*s,h-78*s,150*s,42*s,"SEARCH [/]","search");this.button(left+330*s,h-78*s,130*s,42*s,"< PAGE","page-prev");this.button(left+475*s,h-78*s,130*s,42*s,"PAGE >","page-next");}c.fillStyle="#afa58e";c.font=`${13*s}px monospace`;c.fillText(`PAGE ${scene.page+1} / SEARCH ${editing?$("sceneTextBridge").value:scene.search||"none"} / ARROWS MOVE / ENTER ACTIVATE / TAB EXITS`,left,h-92*s);}
  drawHome(){const s=this.header("HERMES / MEMORY PALACE","WORLD_SPACE_CONTROL_FOLIO / accepted intent is not observed completion"),w=this.overlay.width,h=this.overlay.height;this.controls=[];this.hotspots=[];const items=[["OPERATIONS / 30 COMMANDS","workspace:operations"],["PROJECT PLANS / 9 ACTIONS","workspace:plans"],["PLANNING ASSISTANCE","workspace:assistance"],["RESOURCES AND EVIDENCE","workspace:evidence"],["TRANSPORT / DATA VIEW","workspace:transport"],["DASHBOARD DIRECTORY","workspace:directory"],["COMMAND RECEIPTS","workspace:receipts"],["HELP / SEMANTIC MODE","workspace:help"]],bw=Math.min(470*s,w*.42),bh=50*s,x=(w-bw)/2,y=125*s;items.forEach(([label,action],i)=>this.button(x,y+i*(bh+8*s),bw,bh,label,action));const c=this.ctx;c.fillStyle="#afa58e";c.font=`${13*s}px monospace`;c.fillText("WASD ORBIT / +/- ZOOM / ARROWS SELECT / ENTER ACTIVATE / TAB LEAVES CANVAS",35*s,h-25*s);}
  ...
  drawPlanDetail(){const detail=snapshot.planDetail,ledger=detail?.ledger||{},revision=detail?.revision||{},editing=scene.bridge?.kind?.startsWith("plan-notes:"),note=editing?$("sceneTextBridge").value:displayPlanNotes(),s=this.header(`PLAN / ${ledger.planId||"none"}`,editing?`EDITING SCOPED NOTES${scene.bridge.focusLost?" / FOCUS LOST - DRAFT RETAINED":""}`:`state ${ledger.state||"unknown"} / exact revision ${revision.revision||"?"} / version ${ledger.version||"?"}`),w=this.overlay.width,h=this.overlay.height,c=this.ctx;this.controls=[];this.hotspots=[];c.fillStyle="#afa58e";c.font=`${12*s}px monospace`;c.fillText(`DIGEST ${revision.contentDigest||"not loaded"}`,45*s,135*s);c.fillText(`DECISION NOTES ${String(note||"none").slice(0,70)}`,45*s,155*s);const actions=PROJECT_PLAN_ACTIONS.filter((item)=>item!=="project-plan.create");actions.forEach((action,index)=>this.button(45*s+(index%2)*310*s,(175+Math.floor(index/2)*55)*s,290*s,43*s,action.replace("project-plan.",""),`plan-action:${action}`));this.textBlock(revision.content||{},680*s,175*s,16,65);if(editing){this.button(45*s,h-75*s,150*s,40*s,"SAVE NOTES","bridge-save");this.button(210*s,h-75*s,150*s,40*s,"CANCEL","bridge-cancel");}else this.button(45*s,h-75*s,130*s,40*s,"BACK","workspace:plans");}
  ...
  drawHelp(){const rows=["ARROWS: move scene focus (never traps Tab)","ENTER: activate focused scene control","W/A/S/D: orbit architecture and physical folio","+/-: zoom; 0: reset view","/: edit search; PageUp/PageDown: page","E: edit current payload or content","Ctrl+Enter or scene Save; Escape or scene Cancel","Accepted intent is not observed completion",{label:"OPEN SEMANTIC MODE [M]",action:"semantic"}].map((item)=>typeof item==="string"?{label:item}:item);this.drawRows("HELP",rows);}
  ...
  render(){this.dirty=false;this.resize();const needsUpload=this.overlayDirty,gl=this.gl,vp=this.camera();gl.viewport(0,0,this.canvas.width,this.canvas.height);gl.clearColor(.025,.034,.026,1);gl.clear(gl.COLOR_BUFFER_BIT|gl.DEPTH_BUFFER_BIT);gl.enable(gl.DEPTH_TEST);gl.enable(gl.CULL_FACE);gl.useProgram(this.archProgram);gl.uniformMatrix4fv(this.vpLocation,false,vp);gl.bindVertexArray(this.vao);gl.drawElementsInstanced(gl.TRIANGLES,36,gl.UNSIGNED_SHORT,0,this.instances.length/9);if(needsUpload)this.drawOverlay();gl.enable(gl.DEPTH_TEST);gl.disable(gl.CULL_FACE);gl.enable(gl.BLEND);gl.blendFunc(gl.SRC_ALPHA,gl.ONE_MINUS_SRC_ALPHA);gl.useProgram(this.overlayProgram);gl.uniformMatrix4fv(this.folioVpLocation,false,vp);gl.bindTexture(gl.TEXTURE_2D,this.overlayTexture);gl.pixelStorei(gl.UNPACK_PREMULTIPLY_ALPHA_WEBGL,true);if(!this.textureAllocated){gl.texImage2D(gl.TEXTURE_2D,0,gl.RGBA,gl.RGBA,gl.UNSIGNED_BYTE,this.overlay);this.textureAllocated=true;}else if(needsUpload){gl.texSubImage2D(gl.TEXTURE_2D,0,0,0,gl.RGBA,gl.UNSIGNED_BYTE,this.overlay);}gl.drawArrays(gl.TRIANGLES,0,6);gl.disable(gl.BLEND);}
  folioPoint(clientX,clientY){const rect=this.canvas.getBoundingClientRect(),ndcX=(clientX-rect.left)/rect.width*2-1,ndcY=1-(clientY-rect.top)/rect.height*2,{eye,target}=this.cameraState(),forward=[target[0]-eye[0],target[1]-eye[1],target[2]-eye[2]],fl=Math.hypot(...forward);forward[0]/=fl;forward[1]/=fl;forward[2]/=fl;let right=[forward[2],0,-forward[0]],rl=Math.hypot(...right);if(rl<1e-6)return null;right=right.map((value)=>value/rl);const up=[right[1]*forward[2]-right[2]*forward[1],right[2]*forward[0]-right[0]*forward[2],right[0]*forward[1]-right[1]*forward[0]],tan=Math.tan(Math.PI/6),aspect=this.canvas.width/this.canvas.height,dir=[forward[0]+right[0]*ndcX*tan*aspect+up[0]*ndcY*tan,forward[1]+right[1]*ndcX*tan*aspect+up[1]*ndcY*tan,forward[2]+right[2]*ndcX*tan*aspect+up[2]*ndcY*tan];if(Math.abs(dir[2])<1e-6)return null;const t=(4-eye[2])/dir[2];if(t<=0)return null;const wx=eye[0]+dir[0]*t,wy=eye[1]+dir[1]*t;if(wx< -8||wx>8||wy< -1||wy>8)return null;return {x:(wx+8)/16*this.overlay.width,y:(8-wy)/9*this.overlay.height};}
  pick(clientX,clientY){const point=this.folioPoint(clientX,clientY);if(!point)return;const hit=[...this.hotspots].reverse().find((item)=>point.x>=item.x&&point.x<=item.x+item.w&&point.y>=item.y&&point.y<=item.y+item.h);if(hit){this.focus=hit.index;this.invalidate();safeScene(()=>handleSceneAction(hit.action));}}
  keyboard(event){if(event.key==="Tab")return false;if(scene.bridge)return false;if(event.key==="ArrowDown"||event.key==="ArrowRight"){this.focus=Math.min(this.controls.length-1,this.focus+1);}else if(event.key==="ArrowUp"||event.key==="ArrowLeft"){this.focus=Math.max(0,this.focus-1);}else if(event.key==="Enter"){return void safeScene(()=>handleSceneAction(this.controls[this.focus]?.action));}else if(event.key==="PageDown"){scene.page+=1;}else if(event.key==="PageUp"){scene.page=Math.max(0,scene.page-1);}else if(event.key==="/"){startBridge("search",scene.search);return true;}else if(event.key.toLowerCase()==="e"){if(scene.workspace==="operation-detail")startBridge("operation",scene.operationDraft);else if(scene.workspace==="plan-editor")startBridge("plan",scene.planDraft);else return false;return true;}else if(event.key.toLowerCase()==="a"){this.yaw-=.12;}else if(event.key.toLowerCase()==="d"){this.yaw+=.12;}else if(event.key.toLowerCase()==="w"){this.pitch=Math.max(-.12,this.pitch-.1);}else if(event.key.toLowerCase()==="s"){this.pitch=Math.min(1.05,this.pitch+.1);}else if(event.key==="+"||event.key==="="){this.distance=Math.max(17,this.distance-2);}else if(event.key==="-"){this.distance=Math.min(52,this.distance+2);}else if(event.key==="0"){this.yaw=-.55;this.pitch=.34;this.distance=31;}else if(event.key==="Escape"){scene.workspace="home";scene.confirm=null;}else return false;event.preventDefault();this.invalidate();return true;}
}
...
$("operationForm").addEventListener("submit",(event)=>{event.preventDefault();safeScene(async()=>{const type=$("operationType").value,form=new FormData(event.currentTarget),extra=JSON.parse(String(form.get("extra")||"{}")),candidate={...record(extra),runId:form.get("runId"),text:form.get("text"),objective:form.get("objective"),repoPath:form.get("repoPath"),baseRef:form.get("baseRef"),sourceIterationId:form.get("sourceIterationId"),sourceRunId:form.get("sourceRunId"),acceptanceGateIds:lines(form.get("gateIds")),artifacts:lines(form.get("artifacts")),targetGenerations:Number(form.get("targetGenerations")),id:form.get("recordId"),itemId:form.get("recordId"),adviceId:form.get("recordId"),gateId:form.get("recordId")};const payload=normalizeOperationPayload(type,candidate);if(!await semanticConfirm(type,{type,payload}))return;await executeOperation(type,payload);});});
$("planForm").addEventListener("submit",(event)=>{event.preventDefault();safeScene(async()=>{const form=event.currentTarget,request=planRequest(form.elements.action.value,form.elements.content.value,form.elements.notes.value);if(!await semanticConfirm(request.type,request))return;await executePlan(request.type,request.payload,request.expectedVersion);});});
function semanticConfirm(title,envelope){return new Promise((resolve)=>{pendingSemanticConfirmation=resolve;$("confirmTitle").textContent=`Confirm ${title}`;$("confirmPayload").textContent=json(envelope);$("confirmCheck").checked=false;$("confirmDialog").showModal();});}
$("confirmDialog").addEventListener("close",()=>{if(!pendingSemanticConfirmation)return;pendingSemanticConfirmation($("confirmDialog").returnValue==="send"&&$("confirmCheck").checked);pendingSemanticConfirmation=null;});
...
$("planForm").elements.content.addEventListener("input",(event)=>{scene.planDraft=event.target.value;});
...
renderer=new PalaceRenderer($("palace"));renderer.start();
