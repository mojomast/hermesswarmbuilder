...
const WORLD_LABELS = Object.freeze([
  Object.freeze({ title: "RUN VAULT", detail: "runs + resources", position: [-10.2, 7.25] }),
  Object.freeze({ title: "SCRIBE DESKS", detail: "agents", position: [-10.2, 5.55] }),
  Object.freeze({ title: "CHRONICLE WALL", detail: "events + audit", position: [-10.2, 3.85] }),
  Object.freeze({ title: "IMPLEMENT RACK", detail: "tools", position: [-10.2, 2.15] }),
  Object.freeze({ title: "JUDGMENT PORTAL", detail: "gates + queue", position: [10.2, 7.25] }),
  Object.freeze({ title: "CARTOGRAPHY TABLE", detail: "plans", position: [10.2, 5.55] }),
  Object.freeze({ title: "TURNING STAIR", detail: "iterations", position: [10.2, 3.85] }),
  Object.freeze({ title: "COURIER DOCK", detail: "transport + receipts", position: [10.2, 2.15] }),
  Object.freeze({ title: "COMMAND FOLIO", detail: "control folio", position: [0, 9.2] }),
  Object.freeze({ title: "ACTIVE CONTEXT", detail: "selected run", position: [0, -1.15], context: true })
]);
...
class PalaceRenderer {
  constructor(canvas){const camera=FOLIO.camera;this.canvas=canvas;this.gl=null;this.available=false;this.lost=false;this.yaw=camera.yawDefault;this.pitch=camera.pitchDefault;this.distance=camera.distanceDefault;this.drag=null;this.pinch=null;this.instances=[];this.hotspots=[];this.controls=[];this.focus=0;this.snapshot=snapshot;this.overlay=document.createElement("canvas");this.ctx=this.overlay.getContext("2d");this.raf=0;this.dirty=true;this.overlayDirty=true;this.textureAllocated=false;this.labelAtlasDirty=true;this.bind();}
  ...
  bind(){
    ...
    this.canvas.addEventListener("pointerdown",(event)=>{this.canvas.setPointerCapture(event.pointerId);this.drag={x:event.clientX,y:event.clientY,moved:false,id:event.pointerId};});
    ...
    this.canvas.addEventListener("pointerup",(event)=>{if(this.drag&&!this.drag.moved)this.pick(event.clientX,event.clientY);this.drag=null;});
  ...
  updateLabelAtlas(){const canvas=this.labelAtlas,c=canvas.getContext("2d"),cellW=canvas.width/2,cellH=canvas.height/5;c.clearRect(0,0,canvas.width,canvas.height);WORLD_LABELS.forEach((label,index)=>{if(label.context&&!currentRunId())return;const x=(index%2)*cellW,y=Math.floor(index/2)*cellH,detail=label.context?`run ${currentRunId()}`:label.detail;c.fillStyle="#10130fee";c.fillRect(x+5,y+7,cellW-10,cellH-14);c.strokeStyle=label.context?"#69c7bd":"#d9af59";c.lineWidth=3;c.strokeRect(x+7,y+9,cellW-14,cellH-18);c.fillStyle="#f2e6c8";c.font="700 25px monospace";c.textAlign="center";c.textBaseline="middle";c.fillText(label.title,x+cellW/2,y+36,cellW-30);c.fillStyle="#b9dcd4";c.font="20px monospace";c.fillText(detail,x+cellW/2,y+70,cellW-30);});const gl=this.gl;gl.bindTexture(gl.TEXTURE_2D,this.labelTexture);gl.pixelStorei(gl.UNPACK_PREMULTIPLY_ALPHA_WEBGL,true);gl.texImage2D(gl.TEXTURE_2D,0,gl.RGBA,gl.RGBA,gl.UNSIGNED_BYTE,canvas);this.labelAtlasDirty=false;}
  ...
  constrainCamera(){const envelope=FOLIO.camera;this.yaw=Math.max(envelope.yawMin,Math.min(envelope.yawMax,this.yaw));this.pitch=Math.max(envelope.pitchMin,Math.min(envelope.pitchMax,this.pitch));this.distance=Math.max(envelope.distanceMin,Math.min(envelope.distanceMax,this.distance));}
  resetCamera(){const camera=FOLIO.camera;this.yaw=camera.yawDefault;this.pitch=camera.pitchDefault;this.distance=camera.distanceDefault;}
  cameraState(){const envelope=FOLIO.camera;this.constrainCamera();const cp=Math.cos(this.pitch),target=FOLIO.center,eye=[target[0]+Math.sin(this.yaw)*cp*this.distance,target[1]+Math.sin(this.pitch)*this.distance,target[2]+Math.cos(this.yaw)*cp*this.distance];return {eye,target,vp:multiply(perspective(envelope.fov,this.canvas.width/this.canvas.height,.1,100),lookAt(eye,target,[0,1,0]))};}
  ...
  render(){this.dirty=false;this.resize();const needsUpload=this.overlayDirty,gl=this.gl,vp=this.camera();gl.viewport(0,0,this.canvas.width,this.canvas.height);gl.clearColor(.025,.034,.026,1);gl.clear(gl.COLOR_BUFFER_BIT|gl.DEPTH_BUFFER_BIT);gl.enable(gl.DEPTH_TEST);gl.depthMask(true);gl.enable(gl.CULL_FACE);gl.useProgram(this.archProgram);gl.uniformMatrix4fv(this.vpLocation,false,vp);gl.bindVertexArray(this.vao);gl.drawElementsInstanced(gl.TRIANGLES,36,gl.UNSIGNED_SHORT,0,this.instances.length/9);this.drawWorldLabels(vp);if(needsUpload)this.drawOverlay();gl.enable(gl.DEPTH_TEST);gl.depthMask(true);gl.disable(gl.CULL_FACE);gl.enable(gl.BLEND);gl.blendFunc(gl.SRC_ALPHA,gl.ONE_MINUS_SRC_ALPHA);gl.useProgram(this.overlayProgram);gl.uniformMatrix4fv(this.folioVpLocation,false,vp);gl.uniform3fv(this.folioCenterLocation,FOLIO.center);gl.uniform2f(this.folioSizeLocation,FOLIO.width,FOLIO.height);gl.bindTexture(gl.TEXTURE_2D,this.overlayTexture);gl.pixelStorei(gl.UNPACK_PREMULTIPLY_ALPHA_WEBGL,true);if(!this.textureAllocated){gl.texImage2D(gl.TEXTURE_2D,0,gl.RGBA,gl.RGBA,gl.UNSIGNED_BYTE,this.overlay);this.textureAllocated=true;}else if(needsUpload){gl.texSubImage2D(gl.TEXTURE_2D,0,0,0,gl.RGBA,gl.UNSIGNED_BYTE,this.overlay);}gl.drawArrays(gl.TRIANGLES,0,6);gl.disable(gl.BLEND);}
  folioPoint(clientX,clientY){const rect=this.canvas.getBoundingClientRect(),ndcX=(clientX-rect.left)/rect.width*2-1,ndcY=1-(clientY-rect.top)/rect.height*2,{eye,target}=this.cameraState(),[fx,fy,fz]=FOLIO.center;const forward=[target[0]-eye[0],target[1]-eye[1],target[2]-eye[2]],fl=Math.hypot(...forward);forward[0]/=fl;forward[1]/=fl;forward[2]/=fl;let right=[-forward[2],0,forward[0]],rl=Math.hypot(...right);if(rl<1e-6)return null;right=right.map((value)=>value/rl);const up=[right[1]*forward[2]-right[2]*forward[1],right[2]*forward[0]-right[0]*forward[2],right[0]*forward[1]-right[1]*forward[0]],tan=Math.tan(FOLIO.camera.fov/2),aspect=this.canvas.width/this.canvas.height,dir=[forward[0]+right[0]*ndcX*tan*aspect+up[0]*ndcY*tan,forward[1]+right[1]*ndcX*tan*aspect+up[1]*ndcY*tan,forward[2]+right[2]*ndcX*tan*aspect+up[2]*ndcY*tan];if(Math.abs(dir[2])<1e-6)return null;const t=(fz-eye[2])/dir[2];if(t<=0)return null;const wx=eye[0]+dir[0]*t,wy=eye[1]+dir[1]*t,x0=fx-FOLIO.width/2,y0=fy-FOLIO.height/2;if(wx<x0||wx>x0+FOLIO.width||wy<y0||wy>y0+FOLIO.height)return null;return {x:(wx-x0)/FOLIO.width*this.overlay.width,y:(y0+FOLIO.height-wy)/FOLIO.height*this.overlay.height};}
  ...
  keyboard(event){
    ...
    if(event.key==="ArrowDown"||event.key==="ArrowRight")this.focus=Math.min(this.controls.length-1,this.focus+1);
