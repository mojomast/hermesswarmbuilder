...
var BaseTransitionImpl = {
  ...
  setup(props, { slots }) {
    ...
    return () => {
      ...
      const child = children && children.length ? findNonCommentChild(children) : instance.subTree ? createCommentVNode() : undefined;
      if (!child) {
        return;
      }
      ...
      if (props) {
        {
          ...
          const namespace = el.namespaceURI.includes("svg") ? "svg" : el.namespaceURI.includes("MathML") ? "mathml" : undefined;
...
function createBaseVNode(type, props = null, children = null, patchFlag = 0, dynamicProps = null, shapeFlag = type === Fragment ? 0 : 1, isBlockNode = false, needFullChildrenNormalization = false) {
  ...
  if (props && vnode.shapeFlag & 1) {
    ...
    if (overwritingProp && hasContentChildren(vnode.children)) {
      warn$1(`The \`${overwritingProp}\` prop on <${vnode.type}> will override its children. Remove either the \`${overwritingProp}\` prop or the children.`);
    }
  }
  ...
  _resolveProps(def2) {
    ...
    for (const key of declaredPropKeys.map(camelize)) {
      if (key in Object.getPrototypeOf(this)) {
        warn(`Custom element prop "${key}" conflicts with an existing property on the element and will overwrite it.`);
      }
      Object.defineProperty(this, key, {
        get() {
...
function validateBrowserExpression(node, context, asParams = false, asRawStatements = false) {
  ...
  try {
    ...
    context.onError(createCompilerError(46, node.loc, undefined, message));
  }
}
  ...
  if (node.type === 5) {
    ...
    for (let i = 0;i < node.props.length; i++) {
      ...
      if (dir.type === 7 && dir.name !== "for") {
        const exp = dir.exp;
        const arg = dir.arg;
        if (exp && exp.type === 4 && !(dir.name === "on" && arg) && !(memo && context.vForMemoKeyedNodes.has(node) && arg && arg.type === 4 && arg.content === "key")) {
...
var transformElement = (node, context) => {
  return function postTransformElement() {
    ...
    const vnodeCall = node.codegenNode = createVNodeCall(context, vnodeTag, vnodeProps, vnodeChildren, patchFlag === 0 ? undefined : patchFlag, vnodeDynamicProps, vnodeDirectives, !!shouldUseBlock, false, isComponent2, node.loc);
...
var transformVBindShorthand = (node, context) => {
  if (node.type === 1) {
    for (const prop of node.props) {
      if (prop.type === 7 && prop.name === "bind" && (!prop.exp || prop.exp.type === 4 && !prop.exp.content.trim()) && prop.arg) {
        const arg = prop.arg;
