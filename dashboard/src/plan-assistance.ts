...
function serverPrompt(conversation: any, message: string) {
  ...
  const allowed = "pipelineType, title, problem, intendedUsers, objective, boundedScope, requirements, nonGoals, constraints, risks, repository {path, baseRef, baseCommit}, acceptanceGates {id, description, severity, required, requiredEvidence}, scopeBundles {id, description, capabilities, acceptanceGateIds}, validationPolicy {id, expectations, clientCommandsAllowed}, milestones, limits, lineage";
  return `You are a planning conversation assistant for an autonomous project builder.\n` +
    ...
    `The server-authorized pipeline is ${conversation.pipelineType}. A proposedContent, when useful, should be the full apb.project-plan.v1 content object and may contain only these existing fields: ${allowed}. Use validationPolicy exactly {"id":"apb.runner-selected.v1","expectations":[],"clientCommandsAllowed":false}; use gate severity only "must" or "should"; use limits maxIterations=1, maxVariantsPerIteration=1, maxParallelVariants=1, maxAcceptedFeatures=1, maxVisualMotifChanges=0, maxNewSections=0, stopAfterNoImprovement=1. maxAcceptedFeatures counts accepted scope bundles, not individual capabilities: for one cohesive MVP with many required capabilities/gates, provide one scopeBundles entry listing every capability and mapped acceptanceGateIds. For managed proposals repository.baseCommit must be null. Never include command, argv, shell, script, executable, environment, tool, hook, terminal, file, web, delegation, or unknown fields.\n` +
...
function parseOutput(raw: string, pipelineType: "classic" | "managed") {
  ...
  try {
    ...
    const prepared = {
      ...
      acceptanceGates,
      scopeBundles: Array.isArray(content.scopeBundles) ? content.scopeBundles : [],
