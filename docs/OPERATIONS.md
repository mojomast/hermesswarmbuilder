...
Runner subprocesses have finite wall-clock limits. Override them with `APB_COMMAND_TIMEOUT_MS` (default 10 minutes), `APB_CLASSIC_TIMEOUT_MS` (default 2 hours), `APB_VARIANT_TIMEOUT_MS` (default 45 minutes), and `APB_EVALUATOR_TIMEOUT_MS` (default 20 minutes). `APB_STREAM_DRAIN_TIMEOUT_MS` bounds output draining after the immediate process exits, and `APB_TERMINATION_GRACE_MS` controls the TERM-to-KILL grace period (both default to 5 seconds). Values are bounded by the runner. On Linux each subprocess starts in its own process group, so timeout cleanup—and residual descendant cleanup after a stream-drain timeout—sends TERM and then KILL to the group and verifies process-group disappearance. Other platforms can confirm only direct-child exit, not descendant cleanup; if termination cannot be confirmed, the run remains blocked and cannot be recovered into concurrent work. A stream-drain timeout records `runner-stream-drain-truncated` only after cleanup is confirmed. Genuine process timeouts use `runner-timeout`; managed variant, evaluator, and validation-command timeouts preserve structured scope, timeout, exit code 124, blocked lifecycle/handoff, logs, and worktrees. Validation-command results are persisted in variant or synthesis artifacts before blocking. A classic timeout remains blocked on ordinary hourly ticks and ignores pending managed/project launches; only **Run next tick** may recover a verified classic timeout. A managed timeout requires an explicit matching **continue-from-iteration** request; plain run-now cannot recover it.
