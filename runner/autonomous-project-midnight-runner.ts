...
let launchAuthority: LaunchAuthority | null = null;
function authority(): LaunchAuthority { if(!launchAuthority) throw new Error("launch authority is not initialized"); return launchAuthority; }
...
function writeControl(c:any){
  withProjectionLock(ROOT,()=>{
    const current=readJson(CONTROL,{}), currentPointer=current.projectLaunchRequest, incomingPointer=c.projectLaunchRequest;
    ...
    const next={...current,...c,...(preservePointer?{projectLaunchRequest:currentPointer}:{})}; next.schemaVersion="apb.control.v1"; next.updatedAt=now(); writeFileSync(CONTROL, JSON.stringify(next,null,2));
  });
}
...
function validateApprovedProjectContent(content:any,pipelineType:string){
  projectExactKeys(content,["pipelineType","title","problem","intendedUsers","objective","boundedScope","requirements","nonGoals","constraints","risks","repository","acceptanceGates","scopeBundles","validationPolicy","milestones","limits","lineage"],"project plan content");
  ...
  const scopeBundles=content.scopeBundles; if(!Array.isArray(scopeBundles)||scopeBundles.length>50) throw new Error("project plan scopeBundles is malformed"); const scopeBundleIds=new Set<string>();
  for(const bundle of scopeBundles){ projectExactKeys(bundle,["id","description","capabilities","acceptanceGateIds"],"project plan scope bundle"); const id=projectId(bundle.id,"scope bundle id"); if(scopeBundleIds.has(id)) throw new Error(`duplicate scope bundle ${id}`); scopeBundleIds.add(id); if(typeof bundle.description!=="string"||!bundle.description.trim()||!Array.isArray(bundle.capabilities)||!bundle.capabilities.length||bundle.capabilities.some((x:any)=>typeof x!=="string"||!x.trim())||!Array.isArray(bundle.acceptanceGateIds)||new Set(bundle.acceptanceGateIds).size!==bundle.acceptanceGateIds.length||bundle.acceptanceGateIds.some((id:any)=>!gateIds.has(id))) throw new Error(`scope bundle ${id} is malformed or references an unknown acceptance gate`); }
...
function updateProjectLaunch(binding:ProjectLaunchBinding,status:"running"|"paused"|"blocked"|"completed",runId:string,iterationId:string|null=null,extra:any={}){
  const result=status==="running"?authority().claim(binding.identity,runId,iterationId):authority().transition(binding.identity,status,runId,iterationId,extra);
  if(status==="running"&&result.status==="already-claimed"&&result.record.runId!==runId) throw new Error("project launch was already claimed");
}
...
function resolveIterationRequest(control:any, state:any, queue:any): any | null {
  ...
  return {
    ...
    scopeBundles: Array.isArray(source.scopeBundles)?source.scopeBundles.map((bundle:any)=>({...bundle,capabilities:Array.isArray(bundle?.capabilities)?[...bundle.capabilities]:[],acceptanceGateIds:Array.isArray(bundle?.acceptanceGateIds)?[...bundle.acceptanceGateIds]:[]})):[],
...
async function streamHermes(runId:string, runRoot:string, agentId:string, cwd:string, query:string, stdoutPath:string, stderrPath:string, extraEnv:Record<string,string>={}){
  ...
  const maxTurns=agentId.startsWith("evaluator-")?clampInt(process.env.APB_EVALUATOR_MAX_TURNS,8,4,16):clampInt(process.env.APB_VARIANT_MAX_TURNS,24,8,30);
...
function variantPrompt(req:any, v:WorktreeVariant, runRoot:string, baseCommit:string){ const gates=(Array.isArray(req.snapshottedAcceptanceGates)?req.snapshottedAcceptanceGates:[]).map((gate:any)=>`- ${gate.id}: ${gate.description||""}; required evidence: ${(gate.requiredEvidence||[]).join(" | ")}`).join("\n")||"- No additional acceptance gates"; const hasPredeclaredBundles=Array.isArray(req.scopeBundles)&&req.scopeBundles.length>0; const bundles=hasPredeclaredBundles?JSON.stringify(req.scopeBundles):"No predeclared bundles; declare one cohesive bundle only for capabilities necessary to this objective."; const bundleContract=hasPredeclaredBundles?`\nPredeclared bundle contract (opaque, case-sensitive identifiers; fail closed): copy every declared bundle id, every declared capability string into its acceptedScopeBundles[].acceptedFeatures, the same capability strings into top-level acceptedFeatures, and every declared acceptanceGateIds string verbatim. No paraphrase, omission, addition, normalization, case change, or undeclared bundle/work is allowed. Exact declared bundle payload: ${bundles}\n`:""; return `You are ${v.id}, one bounded Hermes Autonomous Project Builder variant agent.\nObjective: ${req.objective}\nChange request: ${req.changeText||"(none)"}\nAcceptance gates (implement and test the observable behavior, not just a domain-model claim):\n${gates}\nApproved scope bundles: ${bundles}${bundleContract}\nRepo worktree: ${v.path}\nBase commit: ${baseCommit}\nRules: make one focused, shippable alternative; no unrelated features; no tech-stack churn; keep generated artifacts/logs/build output out of git. Commit your source/test/doc/config changes on this branch.\nArtifact priority: after the first coherent implementation and before spending turns on optional polish, write the required JSON artifact below; update it after final validation. Do not spend your final turns only on prose or testing and omit this artifact.\nBefore exit, write JSON to ${join(runRoot,"artifacts","variants",`${v.id}.json`)} with schemaVersion apb.variant.v1, variantId, title, claim, acceptedFeatures (a non-empty array of product capabilities), acceptedScopeBundles (a non-empty array of {id, acceptedFeatures, acceptanceGateIds}; every accepted feature must occur in exactly one bundle), objectiveMapping (a non-empty object mapping objective names to implementation/evidence strings, or a non-empty array of such mappings), changes (an unconstrained implementation summary), risks, evidence, validationNotes, and budget containing integer visualMotifChanges/newSections plus boolean techStackChurn/unrelatedFeatures. maxAcceptedFeatures limits acceptedScopeBundles, never capability items. The runner will write ${v.id}.diff. Do not write outside the worktree except that artifact JSON.`; }
function evaluatorPrompt(req:any, v:WorktreeVariant, runRoot:string){ return `You are evaluator for ${v.id}. Read ${join(runRoot,"artifacts","variants",`${v.id}.json`)} and ${join(runRoot,"artifacts","variants",`${v.id}.diff`)}. Score objectiveFit, userValue, visualQuality, implementationQuality, accessibility, performance from 0-100 and total 0-100. Hard-reject unrelated features, tech-stack churn, missing tests/evidence. Write ${join(runRoot,"artifacts","evaluations",`evaluation-${v.id}.json`)} with schemaVersion apb.evaluation.v1, variantId, scores, hardGateViolations, recommendation accept|reject|partial, rationale, and evidenceArtifacts. evidenceArtifacts MUST include these exact relative string paths (not absolute paths or objects): artifacts/variants/${v.id}.json and artifacts/variants/${v.id}.diff.`; }
...
function readVariantArtifact(path:string, variantId:string){
  ...
  const bundles=value.acceptedScopeBundles;
  const validBundles=Array.isArray(bundles)&&bundles.length>0&&bundles.every((bundle:any)=>bundle&&typeof bundle==="object"&&!Array.isArray(bundle)&&typeof bundle.id==="string"&&bundle.id.trim()&&Array.isArray(bundle.acceptedFeatures)&&bundle.acceptedFeatures.length>0&&bundle.acceptedFeatures.every((item:any)=>typeof item==="string"&&item.trim())&&Array.isArray(bundle.acceptanceGateIds)&&bundle.acceptanceGateIds.every((id:any)=>typeof id==="string"&&id.trim()))&&new Set(bundles.map((bundle:any)=>bundle.id)).size===bundles.length;
  const bundleFeatures=validBundles?bundles.flatMap((bundle:any)=>bundle.acceptedFeatures):[];
  const uniqueFeatures=Array.isArray(value.acceptedFeatures)&&new Set(value.acceptedFeatures).size===value.acceptedFeatures.length;
  if(value.schemaVersion!=="apb.variant.v1"||value.variantId!==variantId||typeof value.claim!=="string"||!value.claim.trim()||!Array.isArray(value.acceptedFeatures)||value.acceptedFeatures.length===0||!value.acceptedFeatures.every((item:any)=>typeof item==="string"&&item.trim())||!uniqueFeatures||!validBundles||new Set(bundleFeatures).size!==bundleFeatures.length||bundleFeatures.length!==value.acceptedFeatures.length||bundleFeatures.some((feature:any)=>!value.acceptedFeatures.includes(feature))||!validMapping||!summary(value.changes)||!summary(value.evidence)||!value.budget||!Number.isInteger(value.budget.visualMotifChanges)||!Number.isInteger(value.budget.newSections)||typeof value.budget.techStackChurn!=="boolean"||typeof value.budget.unrelatedFeatures!=="boolean") throw new Error(`malformed variant artifact for ${variantId}: ${path}`);
...
async function runManagedIterationLoop(runId:string, runRoot:string, req:any, iterationScaffold:any){
  try{
    ...
    for(const v of variants){
      ...
      const declaredBundles=Array.isArray(req.scopeBundles)?req.scopeBundles:[];
      const allowedGates=new Set((Array.isArray(req.snapshottedAcceptanceGates)?req.snapshottedAcceptanceGates.map((gate:any)=>gate.id):req.acceptanceGateIds)||[]);
      const bundleViolation=v.json.acceptedScopeBundles.length>req.limits.maxAcceptedFeatures||v.json.acceptedScopeBundles.some((bundle:any)=>bundle.acceptanceGateIds.some((id:string)=>!allowedGates.has(id)))||v.json.acceptedScopeBundles.some((bundle:any)=>{ const declared=declaredBundles.find((candidate:any)=>candidate.id===bundle.id); return declaredBundles.length>0&&(!declared||declared.capabilities.length!==bundle.acceptedFeatures.length||declared.capabilities.some((feature:string)=>!bundle.acceptedFeatures.includes(feature))||declared.acceptanceGateIds.length!==bundle.acceptanceGateIds.length||declared.acceptanceGateIds.some((id:string)=>!bundle.acceptanceGateIds.includes(id))); });
      if(bundleViolation||v.json.budget.visualMotifChanges>req.limits.maxVisualMotifChanges||v.json.budget.newSections>req.limits.maxNewSections||v.json.budget.techStackChurn||v.json.budget.unrelatedFeatures) throw new Error(`variant ${v.id} exceeds the contracted scope-bundle, motif, section, or scope budget`);
    }
...
async function main(){
  ...
  try{
    ...
    if(control.projectLaunchRequest?.status==="pending"){
      ...
      catch(err:any){ const reason=err?.message||String(err); const rejectedLaunchId=control.projectLaunchRequest?.launchId; try { if(rejectedLaunchId) authority().rejectRequested(rejectedLaunchId,{rejectedAt:now(),rejectionReason:reason}); } catch { control.projectLaunchRequest={...control.projectLaunchRequest,status:"rejected",rejectedAt:now(),rejectionReason:reason}; writeControl(control); } s.status="blocked"; s.phase="blocked"; s.block={reason:`Project launch rejected before runner work: ${reason}`,since:now(),owner:"midnight-runner",suggestedAction:"Inspect the immutable project plan, approval, launch, ledger, and control pointer; create a newly approved launch after correcting the records."}; s.lastAction="Runner rejected a stale, malformed, tampered, or unapproved project launch pointer before Hermes work."; writeState(s); event("error","system","project-launch-rejected",s.lastAction,{planId:control.projectLaunchRequest?.planId||null,launchId:rejectedLaunchId||null,reason}); log(s.lastAction); return; }
    }
